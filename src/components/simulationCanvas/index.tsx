...
export default function SimulatorCanvas(props: SimulatorCanvasProps) {
  ...
  const light = (ctx: CanvasRenderingContext2D, x: number, y: number, on: boolean, color: string, defaultColor?: string) => {
  ...
  useEffect(() => {
    ...
    ctx.fillText(
      `Servidos: ${served} | Espera média: ${avgWait.toFixed(2)}s`,
      22,
      96
    );
  }, [width, height, counts, phase, subState, timer, simTime, served, avgWait]);
